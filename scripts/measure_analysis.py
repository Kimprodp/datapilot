"""T2-3 측정용 — 분석 N회 연속 실행 + .logs/<run_id>.jsonl 출력.

사용:
    uv run python scripts/measure_analysis.py             # 연속 2회 (캐시 효과 비교)
    uv run python scripts/measure_analysis.py --runs 1    # 단발 1회만
...
def _run_once(repo: DuckDBAdapter, run_idx: int, total: int) -> str:
    print(f"\n{'=' * 60}")
    print(f"[run {run_idx}/{total}] 분석 시작")
    print("=" * 60)

    metrics = MetricsCollector()
    orch = PipelineOrchestrator(repo)
    report = orch.run(GAME_ID, PERIOD, metrics=metrics)
...
def main() -> None:
    ...
    with DuckDBAdapter() as repo:
        for i in range(1, args.runs + 1):
            run_ids.append(_run_once(repo, i, args.runs))
