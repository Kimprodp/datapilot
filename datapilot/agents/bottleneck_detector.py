...
(예: DAU가 감소하면 매출도 자연히 감소하므로, DAU만 이상이고 매출은 정상으로 분류)
   한 지표의 변화가 다른 지표의 자연스러운 결과인 경우 \
(예: 유입↑→단가↓, 성공률↓→매출↓), 두 지표 모두 보고하되 \
reasoning에 "연동 관계"를 명시한다. \
반드시 후행 지표의 severity를 선행 지표보다 한 단계 낮게 설정한다 \
(예: 선행 HIGH → 후행 MEDIUM, 선행 MEDIUM → 후행 LOW).
4. 4~5% 감소라도 7일 이상 연속 하락하면 이상으로 분류한다. \
단일 스파이크보다 추세가 중요하다.
5. 결제 성공률 같은 안정성 지표는 5% 미만 변동이라도 이상으로 본다.
6. 리텐션 지표(D1, D7 등)는 3%p 이상 하락하면 이상으로 분류한다. \
점진적 하락이라도 놓치지 않는다.
...
- reasoning: 3문장 이내. 구조: 시점 명시 -> 변화 패턴 -> 다른 지표와의 대조
